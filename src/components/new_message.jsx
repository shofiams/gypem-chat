...
const NewMessagePopup = ({ isOpen, onClose }) => { // onChatCreated dihapus
  ...
  // Kita BUKAN memanggil refetchRooms dari sini, tapi kita perlu 'rooms' untuk cek
  ...
  const handleContactClick = async (contact) => {
    ...
    try {
      ...
      // 1. Cek dari state 'rooms' yang sudah ada
      ...
      if (existingRoom) {
        ...
        if (isMobile) {
          ...
          // --- AWAL PERUBAHAN ---
          // 2. Kirim event 'setActiveChat' untuk room yang sudah ada
          if (isOnChatPage) {
            window.dispatchEvent(new CustomEvent('setActiveChat', { detail: { chatId: roomId } }));
          } else {
            navigate('/chats');
            setTimeout(() => {
              // setActiveChat(roomId); // Ini juga bisa, tapi event lebih konsisten
              window.dispatchEvent(new CustomEvent('setActiveChat', { detail: { chatId: roomId } }));
            }, 100);
          }
          // --- AKHIR PERUBAHAN ---
      ...
      // 3. Jika tidak ada, buat room baru di server
      ...
      if (result.success && result.data && result.data.room_id) {
        ...
        // 5. Kirim event 'newChatCreated'.
        // Listener di 'list_chat_page' akan menangani refetch DAN setActiveChat
        // untuk menghindari race condition.
        window.dispatchEvent(new CustomEvent('newChatCreated', { detail: { chatId: newRoomId } }));
        
        // 6. Jika di mobile, kita tetap harus navigasi manual
        if (isMobile) {
